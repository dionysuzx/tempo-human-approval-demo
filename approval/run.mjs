...
export async function run(api, event, config, now = Date.now()) {
  ...
  const instructions = !enrolled
    ? `### Register your signing key first\n\nThis repository has not registered your native signing key yet. [Open the signing page](${site.href}) to set it up, then have the repository owner register the public record shown there. No approval can pass until registration is complete.\n\nAfter registration, comment \`/request-approval\` for a signing link.`
    : `### Approve this PR\n\n**⌘-click** [Open the signing page](${link}) in **Brave** to keep this PR open in its own tab. Review the message, then open **Native Signing Bridge** and approve with Touch ID. Your signed proof will be posted here automatically.\n\nCommit: \`${current.head}\` · Link expires ${new Date(request.expires).toISOString()}.\n\nKeep the local signer running. For a new link, comment \`/request-approval\`.`;
