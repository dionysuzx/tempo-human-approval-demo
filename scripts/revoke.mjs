...
if(!['browser','native','iphone'].includes(kind))throw Error('Use npm run revoke -- browser OR npm run revoke -- native OR npm run revoke -- iphone');
...
try{
 execFileSync('gh',['variable','set',`${kind === 'iphone' ? 'NATIVE' : kind.toUpperCase()}_SIGNER_REVOKED`,'--repo',repo,'--body','true'],{stdio:'inherit'});
 console.log(`${kind === 'iphone' ? 'All native (iPhone and Mac)' : kind} authorization revoked for future workflow runs. All demo merges are locked, including previously approved commits.`);
